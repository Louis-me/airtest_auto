...
def run(root_path, test_case, device, log_date, recording=None):
    ...
    log = os.path.join(log_date, log_host_path)
    ...
    try:
        ...
        is_success = False
    return {"is_success": is_success, "output_file": output_file, "script": script, "log": log, "log_host_path": log_host_path}
...
class CustomAirtestCase(AirtestCase):
    ...
    def run_air(self, device, data):
        ...
        if data["remove_log"] and os.path.isdir(log_path):
            ...
            print(str(local_host_path) + 'is created')
        for j in get_data_list:
            ...
            result = {"result": get_run["is_success"], "start_date": st_date,"end_date": end_date, "sum_time": sum_time,
        ...
        print("测试报告为：%s/%s/%s/%s" % (data['report_host'],"log",log_date,put_html))
