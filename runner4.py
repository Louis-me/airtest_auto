...
def run(root_path, test_case, log_date):
    ...
    args = Namespace(device=None,log=log, compress=None, recording=True, script=script)
    try:
...
class CustomAirtestCase(AirtestCase):
    ...
    def run_air(self, data):
        ...
        for j in get_data_list:
            ...
            # 循环运行用例
            get_run = run(data["root_path"], j, log_date)
            # 用例结束执行日期
...
if __name__ == '__main__':
    """
    python D:/project/airtest_auto/runner4.py
    """
    root_path = PATH("air_case")
    # test_plan=1 表示调试用例，0表示全部用例
    # data = {"root_path": root_path, "test_plan": 0, "test_module": []}
    data = {"root_path": root_path, "test_plan": 1, "test_module": ["web"]}
    run_case(data)
